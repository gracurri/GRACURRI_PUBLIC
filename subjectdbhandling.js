...
var search = function(error, results, fields) {
    if (error) {
        res.send({
            "code": 400,
            "error": "error"
...
exports.search = function(key, res) { //info_input과목검색
    ...
    subjectdb.query('SELECT name,id from subject WHERE name LIKE' + subjectdb.escape('%' + key + '%') + ';', search(error, results, fields));
    subjectdb.query('SELECT name,id from subject_1 WHERE name LIKE' + subjectdb.escape('%' + key + '%') + ';', search(error, results, fields));
}